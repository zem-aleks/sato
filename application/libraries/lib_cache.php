...
class Lib_cache{
    ...
    * @return array
    */
    public function module($model, $method, $arguments = array(),$cached = false, $expires = NULL){
        //var_dump($arguments);
        if(!in_array($model, $this->ci->load->_ci_models)){
            $this->ci->load->model('modules/'.$model);
        }
        ...
        if(!empty($method) && method_exists($model,$method)){
            ...
            if($cached){
                ...
                return $this->_callnotcache($model, $method, $arguments);
            }
        }                        
        else return false;
    }
    ...
    public function library($library, $method, $arguments = array(),$cached = false, $expires = NULL){
        
        if(!in_array($library, $this->ci->load->_ci_classes)){
            $this->ci->load->model($library);
        }
        
        if(!empty($method) && method_exists($library,$method)){
            
            if($cached){
                return $this->_call($library, $method, $arguments, $expires);
            }
            else{
                return $this->_callnotcache($library, $method, $arguments);
            }
        }                        
        else return false;
    }
    ...
    public function model($model, $method, $arguments = array(),$cached = false, $expires = NULL){
    ...
    private function _callnotcache($property, $method, $arguments = array()){
        ...
        return call_user_func_array(array($this->ci->$property, $method), $arguments);
    ...
    */
    ...
    function delete_cache_html_file($filename){
    ...
    */
    ...
    function delete_all_html($dirname = ''){
    	
    	$path = BASEPATH.'cache/';
		$this->ci->load->helper('file');
		
		 if (!file_exists($path.$dirname)) return false;
		 
		 return delete_files($path.$dirname,true);
